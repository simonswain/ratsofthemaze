/* globals Vec3, localStorage, Audio */
...
Coldwar.prototype.show_help = function () {
  ...
  var html = '';
  var title = this.scene.title === 'Index' ? 'Rats of the Maze' : this.scene.title;
  html += '<h1>' + title + '</h1>';
...
Coldwar.prototype.paintScene = function () {
  ...
  var title = this.env.views.title;
  var titlefx = this.env.views.titlefx;
  var story = this.env.views.story;
  var ex = this.env.views.ex;
  var fx = this.env.views.fx;
  var gx = this.env.views.gx;

  leds.ctx.clearRect(0, 0, leds.w, leds.h);
  title.ctx.clearRect(0, 0, title.w, title.h);

  // titlefx.ctx.fillStyle = 'rgba(0, 0, 0, 0.1)'
  // titlefx.ctx.fillRect(0, 0, titlefx.w, titlefx.h)
  titlefx.ctx.clearRect(0, 0, title.w, title.h);

  story.ctx.clearRect(0, 0, story.w, story.h);
  ...
  this.scene.paintTitle(this.env.views.fx, this.env.views.gx);
  gx.ctx.save();
...
Coldwar.prototype.layout = function () {
  ...
  html += '<div id="story" class="' + c + '"><canvas id="Story"></canvas></div>';
  html += '<div id="titlefx" class="' + c + '"><canvas id="Titlefx"></canvas></div>';
  html += '<div id="title" class="' + c + '"><canvas id="Title"></canvas></div>';
  html += '<div id="leds" class="' + c + '"><canvas id="Leds"></canvas></div>';
...
Coldwar.prototype.render = function () {
  ...
  ['leds', 'title', 'story', 'titlefx'].forEach(function (key) {
    this.env.views[key] = {};
    var view = this.env.views[key];
    view.wrap = document.getElementById(key, key.substr(0, 1));
    view.el = document.getElementById(key.substr(0, 1).toUpperCase() + key.substr(1));
